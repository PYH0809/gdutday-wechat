...
export default {
  ...
  mutations: {
    ...
    changeAddClassData(state, Playload) {
      ...
      uni.setStorageSync('addClassData', JSON.stringify(state.addClassData))
    }
  },
  getters: {
    ...
    handlePrimeClassData(state) {
      return state.addClassData.map(item => productClassData(deepCopy(item)))
    },
          ...
          data = testConflict({
            parms: {
              start: getters.thisCampusTime[s].start,
                ...
                getters.thisCampusTime[item.courseTime.length / 2 + s - 1].end,
    ...
    handleAddClassData(state, getters) {
      return deepCopy(getters.handlePrimeClassData).map((item, index) => {
        ...
        item.courseWeekAndContent = item.courseWeekAndContent.filter(item => {
    ...
    if (item.examWeek) {
      handleData[item.examWeek - 1][item.examDay - 1].push({
        ...
        color:
          ...
          (color[item.courseName] =
            courseBlockColorList[mark++ % courseBlockColorList.length]),
        date: item.examDate,
        splitdate:
          item.examDate.split('-')[1] + '-' + item.examDate.split('-')[2],
      ...
      function MakeClass(content) {
        ...
        this.begin = parseInt(time[0] == '0' ? time[1] : time[0] + time[1])
